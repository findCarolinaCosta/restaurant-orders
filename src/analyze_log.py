...
def get_csv_data(path_to_file):
    ...
    try:
        with open(path_to_file, 'r', encoding='utf8') as file:
            return list(csv.DictReader(file, fieldnames=['customer', 'dish', 'days'], delimiter=','))
...
class ClientsInfo():
    ...
    def get_most_requested_dish(self, data):
        return Counter([info['dish'] for info in data if info['customer'] == self.name]).most_common()[0][0]
    
    def get_how_many_times_order_of_a_dish(self, data, dish):
        return Counter([info['dish'] for info in data if info['customer'] == self.name and info['dish'] == dish]).most_common()[0][1]
...
def analyze_log(path_to_file):
    ...
    many_times_order_of_hamburguer = arnaldo.get_how_many_times_order_of_a_dish(data, 'hamburguer')
    ...
    days_joao_never_went_to_the_cafeteria = joao.get_days_never_went_to_the_cafeteria(data)
    ...
    with open('data/mkt_campaign.txt', 'w', encoding='utf-8') as file:
        result = (
            ...
            + f"{days_joao_never_went_to_the_cafeteria}\n"
        )
        file.write(result)
